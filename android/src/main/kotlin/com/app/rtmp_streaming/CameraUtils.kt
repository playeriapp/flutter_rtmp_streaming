...
private fun selectClosestSize(
    ...
    /**
     * Selects the camera input size independently from the encoded output size.
     *
     * For widescreen presets, open the camera with the closest supported 4:3
     * SurfaceTexture size. CameraNativeView then center-crops that texture to
     * the requested encoder ratio before publishing.
     */
    @RequiresApi(Build.VERSION_CODES.LOLLIPOP)
    fun computeBestCameraInputSize(
        activity: Activity?,
        cameraName: String,
        presetArg: ResolutionPreset
    ): Size {
        val available = getCameraResolutions(activity, cameraName)

        if (available.isEmpty()) {
            return computeBestPreviewSize(
                activity,
                cameraName,
                presetArg
            )["size"] as Size
        ...
        val (outputW, outputH) = getTargetSizeForPreset(presetArg)

        if (presetArg == ResolutionPreset.max) {
            return available.maxWithOrNull(CompareSizesByArea())
                ?: available.first()
        }

        val outputAspect = outputW.toDouble() / outputH.toDouble()

        // Low and medium are already 4:3.
        if (outputAspect <= 1.5) {
            return selectClosestSize(available, presetArg)
        }

        val cameraAspect = 4.0 / 3.0
        val fourThreeSizes = available.filter { size ->
            val aspect = size.width.toDouble() / size.height.toDouble()
            kotlin.math.abs(aspect - cameraAspect) / cameraAspect <= 0.02
        }

        if (fourThreeSizes.isEmpty()) {
            Log.w(
                "PlayeriRTMP",
                "No 4:3 camera input for preset=$presetArg; " +
                    "using normal preset selection"
            )
            return selectClosestSize(available, presetArg)
        }

        val desiredInputW = outputW
        val desiredInputH = (outputW * 3.0 / 4.0).toInt()

        val selected = fourThreeSizes.minByOrNull { size ->
            val dw = size.width.toLong() - desiredInputW.toLong()
            val dh = size.height.toLong() - desiredInputH.toLong()
            dw * dw + dh * dh
        } ?: fourThreeSizes.first()

        Log.e(
            "PlayeriRTMP",
            "cameraInput preset=$presetArg selected=${selected.width}x${selected.height} " +
                "outputTarget=${outputW}x${outputH}"
        )
